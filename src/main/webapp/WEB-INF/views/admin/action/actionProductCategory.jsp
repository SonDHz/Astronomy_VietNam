							...
							<div class="card card-box">
								...
								<div class="card-body" id="bar-parent2">
									<form:form id="formSubmit" role="form" class="form-horizontal"
										onsubmit="return validateForm()" modelAttribute="model">
										<div class="form-body">
											<div class="form-group row  margin-top-20">
												<form:hidden path="id" class="form-control" name="id"
													id="id" value="${model.id}" />
												<label class="control-label col-md-3">Tên loại sản
													phẩm: </label>
												<div class="col-md-4">
											...
											<div class="offset-md-3 col-md-9">
												<c:if test="${not empty model.id}">
													<button type="button" class="btn btn-info"
														id="btnAddOrUpdate">Cập nhật loại sản phẩm</button>
												</c:if>
												<c:if test="${empty model.id}">
													<button type="button" class="btn btn-info"
														id="btnAddOrUpdate">Thêm loại sản phẩm</button>
												</c:if>
											</div>
										</div>
									</form:form>
			...
			$
					.ajax({
						...
						success : function(result) {
							window.location.href = "${productcategoryURL}?id="
